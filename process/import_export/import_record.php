...
function check_emp_id_exists($emp_id, $conn)
{
    $sql = "SELECT `emp_id` FROM `t_employee_m` WHERE `emp_id` = :emp_id";
...
function check_csv($file, $category, $conn)
{
    ...
    while (($line = fgetcsv($csvFile)) !== false) {
        ...
        if (!check_emp_id_exists($emp_id, $conn)) {
            $message .= "Employee ID $emp_id doesn't exist on the masterlist. ";
        }
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $message .= "Authorization No. '$auth_no' already exists for Employee No. '$existing_emp_id'. ";
        }
